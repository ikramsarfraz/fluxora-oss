...
- **Sentry** is wired via `withSentryConfig` in `next.config.ts` plus `instrumentation*.ts` and `sentry.*.config.ts`. Source map upload is gated on `SENTRY_AUTH_TOKEN`. New `"use server"` actions should drop a `recordActionBreadcrumb({ action, tenantId, data })` call after auth resolves — see [lib/sentry-scope.ts](lib/sentry-scope.ts). Keep `data` to small structured args (filename, count, ids — not raw payloads, no PII). When the action later throws or a downstream call captures, the breadcrumb ships with the error so a generic 500 in production comes with "which action, which tenant, what got passed in" instead of a bare stack.
